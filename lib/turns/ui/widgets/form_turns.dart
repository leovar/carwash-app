...
class _FormTurns extends State<FormTurns> with SingleTickerProviderStateMixin {
  ...
  void _assignTurnCallback(Invoice _invoiceClose) {
    ...
    Alert(
      ...
      buttons: [
        DialogButton(
          color: Theme.of(context).colorScheme.secondary,
          child: Text('ASIGNAR', style: Theme.of(context).textTheme.labelLarge),
          onPressed: () {
  ...
  void _endInvoiceWash(Invoice _invoiceToEnd) async {
    ...
    if (_invoiceToEnd.startWashing != null && !(_invoiceToEnd.endWash ?? false)) {
      ...
      _blocInvoice
          .saveInvoice(invoice)
          .then((value) => _countEstimateWashTime());
    }
  }
    ...
    if ((_locationData.activeCells??0) > 0) {
      if (_listInvoicesWashing.length > 0) {
        _listInvoicesWashing.forEach((item) {
          ...
          if (_queueWashingCellsList
                  .where((e) => e.cell == item.washingCell)
                  .length ==
              0) {
            ...
            QueueModel queueTime = _queueWashingCellsList.firstWhere(
              (e) => e.cell == item.washingCell,
            );
            queueTime.time =
                queueTime.time +
      ...
      if (_queueWashingCellsList.length > _queueActiveCellsList.length) {
        ...
        _queueActiveCellsList.forEach((element) {
          if (_queueWashingCellsList
                  .where((e) => e.cell == element.cell)
                  .length >
              0) {
            QueueModel queueWashingTime = _queueWashingCellsList.firstWhere(
              (e) => e.cell == element.cell,
            );
            QueueModel queueActiveTime = _queueActiveCellsList.firstWhere(
              (e) => e.cell == element.cell,
            );
      ...
      if (_listInvoicesWaiting.length > 0) {
        ...
        _listInvoicesWaiting.forEach((item) {
          ...
          QueueModel queueTime = _queueActiveCellsList.firstWhere(
            (e) => e.cell == min.cell,
          );
          queueTime.time =
              queueTime.time +
      ...
      _estimatedWaitingTime = Duration(
        minutes: minTime.time < 0 ? 0 : minTime.time,
      );
    } else {
  ...
  void _completeAssignActiveCells() async {
    ...
    _locationBloc.getLocationById(_idLocation).then((loc) {
      setState(() {
        _locationData = loc;
      });
    });
